...
namespace AuthService.Endpoints.Controllers
{
    public class Controller_Rights_Internal
    {
        ...
        public bool GrantFinalRightToAuthMethod(out bool _bInternalErrorOccured, bool _bDoNotGetDBClearance, string _UserID, string _AuthMethodKey, string _PathRegex, List<string> _Rights, Action<string> _ErrorMessageAction)
        {
            _bInternalErrorOccured = false;

            var Endpoint = "http://localhost:" + LocalServerPort + "/auth/users/" + _UserID + "/access_methods/" + WebUtility.UrlEncode(_AuthMethodKey) + "/access_rights";
            ...
            try
            {
                ...
                if (!Response.IsSuccessStatusCode)
                {
                    ...
                    _ErrorMessageAction?.Invoke("Error: Controller_Rights_Internal->AddFinalRightsToAuthMethod: Request returned error. Endpoint: " + Endpoint + ", code: " + Response.StatusCode + ", message: " + ResponseString);
                    return false;
                }
            }
            catch (Exception e)
            {
                if (e.InnerException != null && e.InnerException != e)
                {
                    _ErrorMessageAction?.Invoke("Error: Controller_Rights_Internal->AddFinalRightsToAuthMethod->Inner: " + e.InnerException.Message + ", Trace: " + e.InnerException.StackTrace);
                }
                if (e is AggregateException)
                {
                    foreach (var Inner in (e as AggregateException).InnerExceptions)
                    {
                        _ErrorMessageAction?.Invoke("Error: Controller_Rights_Internal->AddFinalRightsToAuthMethod->Aggregate->Inner: " + Inner.Message + ", Trace: " + Inner.StackTrace);
                    }
                }
                _ErrorMessageAction?.Invoke("Error: Controller_Rights_Internal->AddFinalRightsToAuthMethod: Request failed. Message: " + e.Message + ", trace: " + e.StackTrace);
                return false;
            }
            return true;
        }

        public bool GrantFinalRightsToAuthMethod(bool _bDoNotGetDBClearance, string _UserID, string _AccessMethod, JArray _Rights, Action<string> _ErrorMessageAction)
        {
            var Endpoint = "http://localhost:" + LocalServerPort + "/auth/users/" + _UserID + "/access_methods/" + WebUtility.UrlEncode(_AccessMethod) + "/access_rights";
            ...
            try
            {
                ...
                if (!Response.IsSuccessStatusCode)
                {
                    _ErrorMessageAction?.Invoke("Error: Controller_Rights_Internal->GrantFinalRightsToAuthMethod: Request returned error. Endpoint: " + Endpoint + ", code: " + Response.StatusCode + ", message: " + ResponseString);
                    return false;
                }
            }
            catch (Exception e)
            {
                if (e.InnerException != null && e.InnerException != e)
                {
                    _ErrorMessageAction?.Invoke("Error: Controller_Rights_Internal->GrantFinalRightsToAuthMethod->Inner: " + e.InnerException.Message + ", Trace: " + e.InnerException.StackTrace);
                }
                if (e is AggregateException)
                {
                    foreach (var Inner in (e as AggregateException).InnerExceptions)
                    {
                        _ErrorMessageAction?.Invoke("Error: Controller_Rights_Internal->GrantFinalRightsToAuthMethod->Aggregate->Inner: " + Inner.Message + ", Trace: " + Inner.StackTrace);
                    }
                }
                _ErrorMessageAction?.Invoke("Error: Controller_Rights_Internal->GrantFinalRightsToAuthMethod: Request failed. Endpoint: " + Endpoint + ", message: " + e.Message + ", trace: " + e.StackTrace);
                return false;
            }
            return true;
        }

        public bool GrantBaseRightsToFinalRights(bool _bDoNotGetDBClearance, string _UserID, string _AccessMethod, Action<string> _ErrorMessageAction)
        {
            var Endpoint = "http://localhost:" + LocalServerPort + "/auth/users/" + _UserID + "/access_methods/" + WebUtility.UrlEncode(_AccessMethod) + "/access_rights/grant_base_access_rights";
            ...
            try
            {
                using var RequestContent = new StringContent(new JObject().ToString(), Encoding.UTF8, "application/json");

                using var RequestTask = Client.PostAsync(Endpoint, RequestContent);
                RequestTask.Wait();
                ...
                if (!Response.IsSuccessStatusCode)
                {
                    _ErrorMessageAction?.Invoke("Error: Controller_Rights_Internal->GrantBaseRightsToFinalRights: Request returned error. Endpoint: " + Endpoint + ", code: " + Response.StatusCode + ", message: " + ResponseString);
                    return false;
                }
            }
            catch (Exception e)
            {
                if (e.InnerException != null && e.InnerException != e)
                {
                    _ErrorMessageAction?.Invoke("Error: Controller_Rights_Internal->GrantBaseRightsToFinalRights->Inner: " + e.InnerException.Message + ", Trace: " + e.InnerException.StackTrace);
                }
                if (e is AggregateException)
                {
                    foreach (var Inner in (e as AggregateException).InnerExceptions)
                    {
                        _ErrorMessageAction?.Invoke("Error: Controller_Rights_Internal->GrantBaseRightsToFinalRights->Aggregate->Inner: " + Inner.Message + ", Trace: " + Inner.StackTrace);
                    }
                }
                _ErrorMessageAction?.Invoke("Error: Controller_Rights_Internal->GrantBaseRightsToFinalRights: Request failed. Endpoint: " + Endpoint + ", message: " + e.Message + ", trace: " + e.StackTrace);
                return false;
            }
            return true;
        }
